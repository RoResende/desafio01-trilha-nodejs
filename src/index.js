...
app.post('/users',(request, response) => {
  ...
  if(userAlreadyExists){
    return response.status(400).json({error:'User already exists, please use a different username'})
  }
...
app.patch('/todos/:id/done', checksExistsUserAccount, checksExistsUserTodo,(request, response) => {
  ...
  if(todo.done){
    return response.status(400).json({error:'Todo is already marked as done'});
  }
